    ...
    <div class="d-flex justify-space-between align-center html-editor">
      ...
      >
        ...
        <v-btn
          ...
          width="43"
          flat
          :color="editor.isActive('orderedList') ? '#D0D0D0' : ''"
...
const content = reactive({ ...editorProps })
...
function insertVariable(variable: string) {
  editor.value?.commands.insertContent(`<span data-type="mention" class="participant-variable" data-id="${variable}" contenteditable="false">${variable}</span>`)
...
watch(() => editorProps.error, (newVal: boolean) => { validateEditor(newVal) })
